...
只有充电功能的线可以供电，但不会创建 COM 端口。如果确认使用的是数据线，Windows 仍然无法识别串口设备，请安装带数字签名的 [CH343 驱动](ch343-driver-installation.md)。
...
主界面会显示蓝牙设备名称、连接摘要、电量信息、Agent 状态和 Typeless 捕获状态，并提供开始配对、设备设置、状态灯、灯效测试、账户、语音、软件设置和清空入口。

### 账户与语音输入

<p align="center">
  <img src="../assets/user-guide/autoclipboard-voice.webp" alt="AutoClipboard 账户与语音配置" width="520">
</p>

- `账户 / 登录` 用于进入 ZKO 账户与权益流程；打开入口本身不会自动上传音频或开始付费操作。
- 免费版语音直接请求 Windows 系统语音输入，由操作系统管理音频和听写文字；AutoClipboard 不录音，也不读取系统听写文本。
- 高级版使用激活码、账户权益和短期云端令牌。只有用户明确选择并完成所需配置后，才会进入对应的云端语音流程；免费版不会被静默切换为云端上传。
- 可以单独设置语音快捷键以及是否拦截触发按键。截图中的未登录和语音关闭状态来自公开文档使用的干净演示配置。
...
这里展示的是 AutoClipboard `0.3.67` 当前源码渲染出的真实界面，不是文生图界面。截图使用隔离的干净设置和脱敏演示设备名，并禁用了真实设备服务，因此只证明软件布局和可见控件，不代表已经观察到实体手柄连接或硬件效果。编号局部图、每个可见区域的用途、软件与硬件分类以及操作结果，请阅读 [AutoClipboard 软件界面详细说明书](software-interface-manual.md)。
...
4. 按照[CH343 驱动安装与排障指南](ch343-driver-installation.md)操作。
