...
这里展示的是 AutoClipboard `0.3.62` 当前源码渲染出的真实界面，不是文生图界面。截图使用隔离的干净设置并禁用了真实设备服务，因此只证明软件布局和可见控件，不代表已经观察到实体手柄连接或硬件效果。编号局部图、每个可见区域的用途、软件与硬件分类以及操作结果，请阅读 [AutoClipboard 软件界面详细说明书](software-interface-manual.md)。
