**简体中文** | [English](../en/software-interface-manual.md) | [返回完整使用说明](user-guide.md) | [返回仓库首页](../../README.md)

<!-- section:s001 -->
# AutoClipboard 软件界面详细说明书

这份说明书按照当前 AutoClipboard 的真实界面，逐块解释主窗口、语音配置以及 **“设备设置 / IMU 预览”** 窗口。每一个控件都会明确说明：它是只查看状态、只修改电脑软件、会写入手柄硬件、只在本次连接中临时生效，还是涉及 Type-C 固件更新。

本文使用的都是 AutoClipboard 当前源码实际渲染的截图，不是文生图界面。为避免把个人账户、Agent 会话、蓝牙后缀或串口信息写入公开文档，截图使用隔离的干净设置并禁用了真实设备服务；局部图只做编号标记和裁切，不会重绘控件或冒充实体设备验证。

> 截图对应 AutoClipboard `0.3.67`。图中的未登录、空 Agent、脱敏的 `CommunistKB-XXXX` 和未选择串口都是安全演示状态；实际电量、Profile 名称、COM 端口、Agent 数量和蓝牙名称后缀会随用户设备变化。
...
| 2 | 剪贴板与 Typeless 工作区 | 接收文字、自动复制、显示捕获状态，并提供账户、语音和软件设置。 |
...
| `BLE 蓝牙未连接` / `BLE 蓝牙已连接：CommunistKB-XXXX` | 显示 AutoClipboard BLE/GATT 会话当前是否就绪，以及连接到哪一把手柄。 | **仅查看** | 连接后会显示完整的 `CommunistKB-` 加真实 4 位后缀，可用于确认软件连接的是正确实体设备。 |
...
| `开始配对` | 打开配对说明，并可继续进入 Windows 蓝牙设置。 | **软件入口 + 硬件配对** | 先让手柄进入 `PAIR` 状态；使用三主机功能时，应在 `Settings > BLE Hosts` 中选择 `EMPTY` 槽位并开启 120 秒配对窗口。 |
...
`开始配对` 不会替用户生成或追加蓝牙后缀。系统中必须选择手柄实际广播的完整 `CommunistKB-XXXX` 名称。
...
| `账户 / 登录` | 打开 ZKO 账户、登录和权益入口。 | **软件 / 网络入口** | 打开入口本身不会上传音频或开始付费操作；后续登录和权益操作由用户明确执行。 |
| `语音` | 打开免费版与高级版语音配置。 | **软件**；选择高级云端档时涉及网络 | 可以启用语音输入、设置快捷键、选择免费系统听写或已授权的高级云端流程。 |
...
| 界面项目 | 用途 | 改动对象 | 产生的效果 |
| --- | --- | --- | --- |
| `登录 / 账户中心` | 进入 ZKO 账户与权益流程。 | **软件 / 网络入口** | 未登录时只显示入口；不会因为打开窗口就上传音频。 |
| `启用语音输入` | 打开或关闭语音快捷键功能。 | **软件** | 关闭时快捷键不会开始语音输入。 |
| 语音快捷键与“拦截快捷键按键” | 选择触发组合，并决定是否阻止该按键继续传给当前应用。 | **软件** | 可以避免触发键在目标输入框中产生额外字符或动作。 |
| 免费版 | 请求 Windows 系统语音输入。 | **操作系统语音服务** | 音频和听写文字由 Windows 管理；AutoClipboard 不录音、不读取系统听写文本。 |
| 高级版（激活码） | 使用已激活的账户权益和短期云端令牌。 | **软件 + 网络** | 只有用户明确选择并完成配置后才进入云端语音流程；不会从免费版静默切换。 |
...
蓝牙配对、波轮操作、硬件接口、Skill 安装和完整故障排查，请返回[完整中文使用说明书](user-guide.md)。
