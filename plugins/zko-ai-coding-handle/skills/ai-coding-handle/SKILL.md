...
description: Use when AutoClipboard installation or updates, AI coding handle Agent hooks, firmware version checks or updates, D4/V3 device identification, USB/serial/BLE connectivity, Linux btusb autosuspend or MediaTek controller firmware, or AutoClipboard and handle diagnostics need attention.
---
...
- Linux Bluetooth power diagnosis is read-only. Applying or removing an adapter-specific udev power rule changes system configuration and requires a separate explicit confirmation after showing the exact HCI, VID/PID, sysfs path, rule path, and rule content.
- Linux MediaTek controller-firmware diagnosis is read-only. Replacing a file under `/lib/firmware` requires explicit software confirmation after showing the exact HCI, USB VID/PID, selected firmware path, running build, installed build, upstream build, source URL, and SHA-256. That confirmation does not authorize a reboot or controller reset.
...
4. On Linux, when the handle stalls while still showing `LINK`, enters `WAIT` after idle, reconnects, or becomes more stable while actively observed, run `scripts/configure-linux-bluetooth-autosuspend.sh --check`. Follow the Linux Bluetooth power workflow in [references/en/troubleshooting.md](references/en/troubleshooting.md); adapter-specific `power/control=on` is the recommended setting only when the read-only evidence matches.
5. When a Linux MediaTek Bluetooth controller still disconnects after a firmware-file update, follow [references/en/troubleshooting.md](references/en/troubleshooting.md). Prove the active `btmtk` firmware family from the USB controller and running build; never infer it from the PCIe Wi-Fi model, a similar filename, or the user's remembered chip number.
6. For published handle firmware version or update requests, follow [references/en/maintenance-contract.md](references/en/maintenance-contract.md), including its current tested V3 baseline; still treat signed `latest.json` metadata and `firmware-check` as the authority for what is publicly available. For an explicit local source build and flash request, follow [references/en/development-flashing.md](references/en/development-flashing.md). Never guess D4 or V3, a port, or which device to use.
...
- For Linux Bluetooth power repair, target only the uniquely identified `btusb` USB parent and its exact VID/PID. Never globally disable USB autosuspend, change `/sys/module/btusb/parameters/enable_autosuspend`, alter unrelated USB devices, restart BlueZ, clear pairings, or reset the adapter as part of this workflow.
- For Linux MediaTek controller firmware, use only the exact file selected from Linux `btmtk` evidence and the official `kernel-firmware/linux-firmware` source. Back up that file before replacement. Never update MT7922 and MT7925 together "just in case", infer the Bluetooth family from `lspci`, or report success before a later boot shows the new build. Do not reboot, unbind, reload `btusb`, or interrupt other Bluetooth devices without separate authorization.
